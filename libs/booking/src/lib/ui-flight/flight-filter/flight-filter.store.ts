...
import { FlightFilter, initialFlightFilter } from "../../logic-flight";
...
export const FlightFilterStore = signalStore(
  ...
  withComputed(({ filters }) => ({
    latestFilter: computed(() => filters().slice(-1)[0])
  })),
  // Updater
  withMethods(store => ({
    updateInputFilter: (filter: FlightFilter) => patchState(store, () => ({
      inputFilter: filter,
    })),
    updateSelectedFilter: (filter: FlightFilter) => patchState(store, () => ({
      selectedFilter: filter,
    })),
    addFilter: (filter: FlightFilter) => patchState(store, state => ({
      filters: [
  ...
  // Effects
  withMethods(({ inputFilter, addFilter, updateInputFilter, updateSelectedFilter }) => ({
    triggerSearch: () => {
